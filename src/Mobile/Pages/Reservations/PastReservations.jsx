...
const PastReservations = () => {
  const [pastReservations, setPastReservations] = useState([]); // State for past reservations
    ...
    const unsubscribe = onSnapshot(allReservationsRef, (snapshot) => {
      ...
      const reservationsData = snapshot.docs
        ...
        .filter((reservation) => {
          ...
          return reservationDate < today; // Filter past reservations
        });

      setPastReservations(reservationsData); // Update state with past reservations
  ...
  return (
    ...
    >
      {loading ? (
        ...
        <Typography>No past reservations found.</Typography> // Message for empty reservation list
      )}
    </div>
  );
};
